...
import SelectableWeatherItem from './AnalyticsSelect'; 
...
export default function AnalyticsPage() {
  ...
  const onSelectWeatherItem = (selectedItem) => {
    // Handle the selected weather item
    console.log('Selected Weather Item:', selectedItem);
  };
              ...
              <div className="bg-gray-800  rounded-lg shadow-xl col-span-4 p-4">
                <div className="h-full text-blue text-center flex space-x-3 overflow-hidden hover:overflow-x-scroll mb-3">
                  {weekly.list
                    ...
                    .map((forecastItem) => (
                      <SelectableWeatherItem
                        key={forecastItem.dt}
                        forecastItem={forecastItem}
                        onSelect={onSelectWeatherItem}
                      />
                    ))}
                </div>
              </div>
